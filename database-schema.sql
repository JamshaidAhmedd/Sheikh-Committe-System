...
CREATE POLICY "Enable read access for all users" ON daily_statuses 
  FOR SELECT USING (true);

CREATE POLICY "Enable insert for all users" ON members 
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Enable insert for all users" ON daily_statuses 
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Enable update for all users" ON members 
  FOR UPDATE USING (true);

CREATE POLICY "Enable update for all users" ON daily_statuses 
  FOR UPDATE USING (true);

CREATE POLICY "Enable delete for all users" ON members 
  FOR DELETE USING (true);

CREATE POLICY "Enable delete for all users" ON daily_statuses 
  FOR DELETE USING (true);

-- 7. Insert sample members data (30 members with proper payout turns)
...
-- 8. Insert initial daily statuses (one paid status for demonstration)
